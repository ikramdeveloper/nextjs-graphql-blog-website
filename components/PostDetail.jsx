...
const PostDetail = ({ post }) => {
  ...
  return (
    <section className="bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8">
      <figure className="relative overflow-hidden shadow-md mb-6 pb-80">
        <Image
          src={post.featuredImage.url}
          alt={post.title}
          layout="fill"
          className="object-top h-full w-full object-cover rounded-t-lg"
        />
      </figure>

      <article className="px-4 lg:px-0">
        <div className="flex items-center justify-between mb-8 w-full">
          <figure className="flex items-center mb-4 lg:mb-0 w-full lg:w-auto">
            <Image
        ...
        {post.content.raw.children.map((typeObj, index) => {
          const children = typeObj.children.map((child, i) =>
            getContentFragment(i, child.text, child)
          );
